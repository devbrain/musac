...
// G.711 µ-law decompression table (from decoder_aiff_v2)
...
// G.711 A-law decompression table (from decoder_aiff_v2)
...
private:
    size_t decode_ulaw(const uint8_t* input, float* output, size_t samples) {
        for (size_t i = 0; i < samples; ++i) {
            // µ-law uses the lookup table directly
            int16_t pcm_value = ulaw_table[input[i]];
            // Convert 16-bit PCM to float [-1.0, 1.0]
            output[i] = static_cast<float>(pcm_value) / 32768.0f;
        }
        return samples;
    }
    
    size_t decode_alaw(const uint8_t* input, float* output, size_t samples) {
        for (size_t i = 0; i < samples; ++i) {
            // A-law uses the lookup table directly
            int16_t pcm_value = alaw_table[input[i]];
            // Convert 16-bit PCM to float [-1.0, 1.0]
            output[i] = static_cast<float>(pcm_value) / 32768.0f;
        }
        return samples;
