...
public:
    ...
    bool is_compressed() const {
        if (!m_container) return false;
        return m_container->is_aifc() && 
               m_container->get_compression_type() != iff::fourcc("NONE");
    }
    
    const char* get_compression_name() const {
        if (!m_codec) return "None";
        return m_codec->name();
    }
...
decoder_aiff_v3::decoder_aiff_v3() 
    : m_pimpl(std::make_unique<impl>()) {
}

decoder_aiff_v3::~decoder_aiff_v3() = default;

void decoder_aiff_v3::open(io_stream* io) {
    m_pimpl->open(io);
}

const char* decoder_aiff_v3::get_name() const {
    return m_pimpl->get_name();
}

sample_rate_t decoder_aiff_v3::get_rate() const {
    return m_pimpl->get_rate();
}

channels_t decoder_aiff_v3::get_channels() const {
    return m_pimpl->get_channels();
}

std::chrono::microseconds decoder_aiff_v3::duration() const {
    return m_pimpl->duration();
}

bool decoder_aiff_v3::rewind() {
    return m_pimpl->rewind();
}

bool decoder_aiff_v3::seek_to_time(std::chrono::microseconds time) {
    return m_pimpl->seek_to_time(time);
}

bool decoder_aiff_v3::is_compressed() const {
    return m_pimpl->is_compressed();
}

const char* decoder_aiff_v3::get_compression_name() const {
    return m_pimpl->get_compression_name();
}

bool decoder_aiff_v3::accept(io_stream* io) {
    return impl::accept(io);
}

size_t decoder_aiff_v3::do_decode(float* buf, size_t len, bool& call_again) {
